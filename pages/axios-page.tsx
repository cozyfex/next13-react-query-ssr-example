...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const data = await getBoard(page);
...
export default function AxiosPage ({ data }: { data: ListInterface<BoardInterface[]> }) {
    ...
    <Layout>
      ...
      <ul style={{ listStyle: 'none' }}>
        {[...Array(Math.ceil((data?.count || 10) / 10)).keys()].map((_, i) =>
          <li key={i + 1} style={{ float: 'left', cursor: 'pointer', padding: '4px' }}>
            <Link href={`/axios-page?page=${i + 1}`}>
              {i + 1}
            </Link>
          </li>,
        )}
